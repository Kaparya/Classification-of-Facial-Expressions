...
**Маппинг эмоций по датасетам:** `calm` (только RAVDESS) и `fru`/`xxx` (только IEMOCAP) исключены для создания единого 7-классового пространства меток.

---

## Гипотезы

**H1 (Эффективность):** Двухэтапная система с аудиогейтингом сокращает общее время GPU примерно на 50% по сравнению с синхронными базовыми линиями за счёт пропуска визуальной обработки на нейтральных сегментах.
...
DistilHuBERT лидирует по качеству, но в 11 раз тяжелее CNN. Log-Mel CNN — лучший компромисс для пайплайна. MFCC+SVM имеет низкий macro F1 из-за дисбаланса классов (нейтральных сэмплов в 5 раз меньше).
...
Результаты отражают строгое speaker-independent разбиение: обучение на 20, тест на 4 актёрах. Малый размер датасета (1200 train / 240 test) при 7 классах ограничивает обобщающую способность.
...
На RAVDESS пайплайн работает хорошо. На IEMOCAP — критическое падение качества: виден сильный domain shift между разыгранной и естественной речью.
