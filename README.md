...
| Сравнение с базовыми линиями (SCME+GF, MM-NodeFormer, ResNet18+GRU) | **В процессе** |
...
Веса сохранены в `trained_models/audio_neutral_{0,1,2}/`, лучшая версия — последняя (`audio_neutral_2`, используется как аудио-гейт).
...
| **EmotiEffNet** (`enet_b0_8_best_vgaf`) | 1280 | ~15 МБ |

### Классификационные модели (темпоральные агрегаторы)

| Голова | Описание |
|--------|----------|
| **BiGRU** | Двунаправленный двухслойный GRU; агрегация по последним hidden states |
| **BiLSTM** | Двунаправленный LSTM, обрабатывающий последовательность кадров |
| **TCN** | Temporal Convolutional Network с расширяющимися 1D-свёртками (dilations 1, 2, 4) и глобальным усреднением по времени |

Итого **6 комбинаций** (2 Backbone × 3 головы). Веса сохранены в `trained_models/video_emotion_2/` (`emotieffnet_b0_backbone.pt` + `head_{mobilenet,emotieffnet}_{bigru,bilstm,tcn}.pt`).
...
**H3 (Качество видео):** Легковесные архитектуры (MobileNetV3-Small) с правильным темпоральным агрегированием (BiGRU/BiLSTM/TCN) достигают сопоставимого с тяжёлыми моделями (EmotiEffNet) качества при значительно меньшей латентности.

**H4 (Сравнение с SOTA):** Предложенная каскадная схема (MelCNN-gate + EmotiEffNet+TCN) на IEMOCAP сопоставима по качеству с современными мультимодальными архитектурами (SCME+GF, MM-NodeFormer) и базовой линией ResNet18+GRU при существенно меньшем размере и времени инференса.
...
|--------|----------|----------|----------|------------------|
> Ниже — цифры прошлой версии (EfficientNet-B0 + Attention/BiLSTM/Transformer). Актуальные результаты для EmotiEffNet + BiGRU/BiLSTM/TCN будут обновлены после завершения перепрогонов.

| Модель (old) | Accuracy | F1 macro | Инференс | Размер пайплайна |
|--------|----------|----------|----------|------------------|
...
### Бенчмарк на IEMOCAP (Session 1–4 train / Session 5 test, 5 классов)

| Модель | Accuracy | F1 weighted | мс/сэмпл | Размер (МБ) |
|--------|----------|-------------|----------|-------------|
...
| Ours (MelCNN+EmotiEffNet+BiLSTM) | 0.3127 | 0.2686 | 3.28 | 13.36 |
